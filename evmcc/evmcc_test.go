...
-----END CERTIFICATE-----`
		...
		It("will create and store the runtime bytecode from the deploy bytecode and a user account", func() {
			...
			//
			// TODO get rid of strict ordering, by using user0cert as user key and
			// res.payload as contract key.
			...
			Expect(key).To(Equal(string(res.Payload)))
			...
			Expect(key).ToNot(Equal(userAddr))
...
-----END CERTIFICATE-----`
			...
			BeforeEach(func() {
				...
				// First PutState Call is to store contract runtime bytecode
				key, value := stub.PutStateArgsForCall(0)
				contractAcct, err := acm.Decode(value)
				Expect(err).ToNot(HaveOccurred())
			...
			Context("when user1 is given right to vote", func() {
				...
				Context("when user1 votes for proposal 'a'", func() {
					BeforeEach(func() {
						...
						res := evmcc.Invoke(stub)
						Expect(res.Status).To(Equal(int32(shim.OK)))
						//TODO: new burrow cache causes the contract account and all its associated. New change count is 5: contract account, sender.voted, sender.vote, voteCount1, voteCount2. Is this a check we care about.
