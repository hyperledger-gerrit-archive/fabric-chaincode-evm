...
func (evmcc *EvmChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	evmCache := evm.NewState(state, func(height uint64) []byte {
		...
		// At the time of execute EVMCC does not have access to the block hash
		// The Burrow EVM should not need to call the Block Hash internally and will not affect
		// execution.
