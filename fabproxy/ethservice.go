...
type Transaction struct { // object, or null when no transaction was found:
	...
	// From             string `json:"from"`             // DATA, 20 Bytes - address of the sender. This is generated by EVM Chaincode. Until account generation stabilizes, we are not returning a value.
...
func (s *ethService) GetTransactionByHash(r *http.Request, txID *string, reply *Transaction) error {
	...
	// query transaciton instaead of below queryblock?
	...
	for index, transactionData := range data {
		if transactionData != nil { // can a data be empty? Is this an error?
			...
			if len(args) == 1 && string(args[0]) == "account" {
				break // no more data available to fill the transaction
			}

			if len(args) != 2 {
				return fmt.Errorf("input had an inappropriate number of args, expect one or two, got %q", len(args))
			}
			...
			// First arg is the callee address. If it is zero
			// address, tx was a contract creation.  We don't care
			// about disambiguating between contract deploy and
			// contract invoke. Shove the data directly into the
			// appropriate fields.
			...
			// txn.From can be gotten from the Signature on the Transaction Envelope
			break
		}
	}
