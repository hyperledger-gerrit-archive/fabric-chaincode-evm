...
var _ = Describe("Ethservice", func() {
	...
	Describe("GetTransactionReceipt", func() {
		...
		BeforeEach(func() {
			...
			sampleTransaction, err = GetSampleTransaction([][]byte{[]byte("82373458"), []byte("sample arg 2")}, []byte("sample-response"), eventBytes, "1234")
			Expect(err).ToNot(HaveOccurred())
		...
		Context("when the transaction is creation of a smart contract", func() {
			...
			BeforeEach(func() {
				...
				tx, err := GetSampleTransaction([][]byte{zeroAddress, []byte("sample arg 2")}, contractAddress, []byte{}, "1234")
