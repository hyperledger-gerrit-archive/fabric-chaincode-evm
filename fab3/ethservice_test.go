...
var _ = Describe("Ethservice", func() {
	...
	Describe("Filtering", func() {
		...
		Context("get filter changes default errors", func() {
			It("cannot get a filter when none is installed", func() {
				filter := types.FilterID{ID: 0}
				var ret []interface{}
				Expect(ethservice.GetFilterChanges(&http.Request{}, &filter, &ret)).To(HaveOccurred())
			})
			It("cannot get a logs filter when none is installed", func() {
				filter := types.FilterID{ID: 0}
				var ret []interface{}
