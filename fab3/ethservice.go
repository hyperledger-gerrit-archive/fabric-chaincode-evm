...
func (s *ethService) GetLogs(r *http.Request, args *types.GetLogsArgs, logs *[]types.Log) error {
	...
	var err error
	if args.BlockHash != "" {
		logger.Debugw("looking for block by hash", "hash", args.BlockHash)

		hash, err := hex.DecodeString(args.BlockHash)
		if err != nil {
			return errors.Wrap(err, "failed to parse the block hash")
		}
		logger.Debugw("looking for block by hash", "hash", hash)
		/*			var decBytes []byte
					_, err = base64.StdEncoding.Decode(decBytes, hash)
					if err != nil {
						return errors.Wrap(err, "failed to parse the block hash")
					}
					logger.Debugw("looking for block by hash", "hash", decBytes)*/
