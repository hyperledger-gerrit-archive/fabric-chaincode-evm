...
//
// gorilla RPC is the receiver of these functions, they must all take three pointers, return a single error, and the third pointer must be named 'reply'
//
	...
	*/
	...
	for blockNumber := from; blockNumber <= to; blockNumber++ {
		...
		for transactionIndex, transactionData := range blockData {
			...
			// for fabric transactions, 0 is valid, 1 is invalid, the opposite of how ethereum
			receipt.Status = "0x" + strconv.FormatUint(((1+uint64())%2), 16)
