...
func (s *ethService) GetTransactionReceipt(r *http.Request, txID *string, reply *TxReceipt) error {
	...
	txLogs, err := fabricEventToEVMLogs(respPayload.Events, receipt.BlockNumber, receipt.TransactionHash, receipt.TransactionIndex, receipt.BlockHash, index)
	if err != nil {
		return err
	}
	receipt.Logs = txLogs
...
func fabricEventToEVMLogs(events []byte, blocknumber, txhash, txindex, blockhash, logindex string) ([]Log, error) {
	if events == nil {
		return nil, nil
	}

	chaincodeEvent, err := getChaincodeEvents(events)
	if err != nil {
