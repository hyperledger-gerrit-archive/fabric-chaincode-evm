...
func (s *ethService) GetTransactionReceipt(r *http.Request, txID *string, reply *types.TxReceipt) error {
	...
	txLogs, err := fabricEventToEVMLogs(logger, respPayload.Events, receipt.BlockNumber, receipt.TransactionHash, receipt.TransactionIndex, receipt.BlockHash, types.AddressFilter{}, types.TopicsFilter{})
	if err != nil {
...
func (s *ethService) GetLogs(r *http.Request, args *types.GetLogsArgs, logs *[]types.Log) error {
	...
	for blockNumber := from; blockNumber <= to; blockNumber++ {
		...
		for transactionIndex, transactionData := range blockData {
			...
			logs, err := fabricEventToEVMLogs(logger, respPayload.Events, blkNumber, transactionHash, transactionIndexStr, blockHash, args.Address, args.Topics)
			if err != nil {
				return errors.Wrap(err, "failed to get EVM Logs out of fabric event")
			}
...
func fabricEventToEVMLogs(logger *zap.SugaredLogger, events []byte, blocknumber, txhash, txindex, blockhash string, af types.AddressFilter, tf types.TopicsFilter) ([]types.Log, error) {
	if len(events) == 0 {
		return nil, nil
	}
