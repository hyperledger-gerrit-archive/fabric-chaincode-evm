...
var _ = Describe("", func() {
	...
	Context("Block JSON", func() {
		DescribeTable("Block MarshalJSON",
			func(blk Block, bytes []byte) {
				fmt.Fprintln(GinkgoWriter, blk)
				fmt.Fprintln(GinkgoWriter, string(bytes))
				marshalledData, err := json.Marshal(&blk)
				Expect(err).ToNot(HaveOccurred())
				fmt.Fprintln(GinkgoWriter, string(marshalledData))
				var target Block
				err = json.Unmarshal(marshalledData, &target)
				Expect(err).ToNot(HaveOccurred())
				fmt.Fprintln(GinkgoWriter, target)
				var expectedBlk Block
				err = json.Unmarshal(bytes, &expectedBlk)
				Expect(err).ToNot(HaveOccurred())
				fmt.Fprintln(GinkgoWriter, expectedBlk)
