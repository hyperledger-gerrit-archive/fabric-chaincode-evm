...
func initFlags() {
	...
	fab3Cmd.PersistentFlags().StringVarP(&cfg, "config", "c", "", "Path to a compatible Fabric SDK Go config file. This flag is required if FAB3_CONFIG is not set")
	viper.BindPFlag("config", fab3Cmd.PersistentFlags().Lookup("config"))

	fab3Cmd.PersistentFlags().StringVarP(&user, "user", "u", "", "User identity being used for the proxy (Matches the users' names in the crypto-config directory specified in the config). This flag is required if FAB3_USER is not set")
	viper.BindPFlag("user", fab3Cmd.PersistentFlags().Lookup("user"))

	fab3Cmd.PersistentFlags().StringVarP(&org, "org", "o", "", "Organization of the specified user. This flag is required if FAB3_ORG is not set")
	viper.BindPFlag("org", fab3Cmd.PersistentFlags().Lookup("org"))

	fab3Cmd.PersistentFlags().StringVarP(&ch, "channel", "C", "", "Channel to be used for the transactions. This flag is required if FAB3_CHANNEL is not set")
	...
	fab3Cmd.PersistentFlags().StringVarP(&ccid, "ccid", "i", "evmcc", "ID of the EVM Chaincode deployed in your fabric network. The CCID to be used in by fab3 can also be set by the FAB3_CCID environment variable.")
	...
	fab3Cmd.PersistentFlags().IntVarP(&port, "port", "p", 5000, "Port that Fab3 will be running on. The listening port can also be set by the FAB3_PORT environment variable.")
