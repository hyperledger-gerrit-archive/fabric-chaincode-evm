...
func (evmscc *EvmChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	//get the callee address from args[0]
...
func (evmscc *EvmChaincode) getCode(stub shim.ChaincodeStubInterface, address []byte) pb.Response {
	...
	//get the key (code) stored at that address from the ledger
...
func getCallerAddress(stub shim.ChaincodeStubInterface) (account.Address, error) {
	//get the identity of the submitter of the transaction
	creatorBytes, err := stub.GetCreator()
	if err != nil {
...
func identityToAddr(id []byte) (account.Address, error) {
	//get the next PEM formatted block
	bl, _ := pem.Decode(id)
	if bl == nil {
		return account.ZeroAddress, fmt.Errorf("no pem data found")
	}

	//parse the block to get the x509 certificate 
...
func main() {
	//err := shim.Start(new(EvmChaincode))
	//if err != nil {
