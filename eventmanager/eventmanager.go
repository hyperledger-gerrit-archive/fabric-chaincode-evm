...
//TODO: Move into its own package so fab3 doesn't need to pull in burrow and avoids the proto collision with fabric

// 1 new interfaces for call and log
// 2 keep flush, to batch all evm events in one transaction into one single fabric event
// 3 event type
...
func (evmgr *EventManager) Log(log *exec.LogEvent) error {
	// convert logEvent to Event, and append
