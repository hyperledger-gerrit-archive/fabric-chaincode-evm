...
# we don't use any of these, they just need to exist so pull that one, and tag busybox as the others
.PHONY: docker-images
docker-images:
	docker pull busybox
	@echo "tag Thirdparty Images"
	for IMAGE in couchdb kafka zookeeper; do \
		docker tag busybox hyperledger/fabric-$$IMAGE ; \
	done
	@echo "tag javaenv IMAGE"
	docker tag busybox hyperledger/fabric-javaenv:amd64-latest
