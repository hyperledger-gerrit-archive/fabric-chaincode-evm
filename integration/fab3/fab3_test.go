...
var _ = Describe("Fab3", func() {
	...
	It("implements the ethereum json rpc api", func() {
		...
		resp, err = sendRPCRequest(client, "eth_getBlockByNumber", proxyAddress, rand.Int(), receipt.BlockNumber)
		Expect(err).ToNot(HaveOccurred())
		rBody, err = ioutil.ReadAll(resp.Body)
		Expect(err).ToNot(HaveOccurred())
		logger.Debug(rBody)
		...
		//Expect(blockHashes).To(ContainElement(latestBlock.GetHeader()))
		logger.Debug(latestBlock)
		// hash from block
		// hash from hash list blockfilter changes
	...
	It("implements the ethereum json rpc api for async-logs", func() {
		...
		By("sucessfully uninstalling the installed block filter")
		...
		By("sucessfully uninstalling the installed filter")
