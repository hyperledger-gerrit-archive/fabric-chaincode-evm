...
var _ = Describe("Fabproxy Integration", func() {
	...
	It("implements the ethereum json rpc api", func() {
		...
		expectedArrayBody := jsonRPCArrayResponse{JsonRPC: "2.0", ID: 5}

		rBody, err := ioutil.ReadAll(resp.Body)
		Expect(err).ToNot(HaveOccurred())

		var respArrayBody jsonRPCArrayResponse
		err = json.Unmarshal(rBody, &respArrayBody)
		Expect(err).ToNot(HaveOccurred())

		Expect(respArrayBody.Error).To(Equal(jsonRPCError{}))
		...
		expectedBody := jsonRPCResponse{JsonRPC: "2.0", ID: 6}
		rBody, err = ioutil.ReadAll(resp.Body)
		Expect(err).ToNot(HaveOccurred())
		...
		Expect(respBody.Error).To(Equal(jsonRPCError{}))
		...
		var rpcResp jsonRPCTxReceipt
		err = json.Unmarshal(rBody, &rpcResp)
		Expect(err).ToNot(HaveOccurred())

		Expect(rpcResp.Error).To(Equal(jsonRPCError{}))
