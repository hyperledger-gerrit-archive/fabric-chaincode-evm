...
var _ = Describe("Web3 Integration", func() {
	...
	It("web3 can deploy and interact with smart contracts", func() {
		...
		pathCmd := exec.Command("echo", "$PATH")
		output, err := pathCmd.CombinedOutput()
		Expect(err).ToNot(HaveOccurred())
		fmt.Println("ECHO CMD: ", string(output))

		fmt.Println("REMOVE ME")
		lsCmd := exec.Command("ls", "/home/jenkins/.nvm/versions/node/8.11.3/bin")
		output, err = lsCmd.CombinedOutput()
		// Expect(err).ToNot(HaveOccurred())
		fmt.Println("LS CMD: ", string(output))

		path := os.Getenv("PATH")
		fmt.Println("REMOVE ME: OS GET ENV PATH: ", path)
