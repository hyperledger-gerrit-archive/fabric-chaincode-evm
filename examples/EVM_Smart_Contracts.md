...
The EVM is installed into Fabric as a chaincode and then smart contracts can
...
consensus. This integration simply adds the smart contract engine. All transactions
will still follow the execute, order, validate steps in the Fabric transaction
flow. Be sure to install the chaincode on enough peers in different orgs and set
an endorsement policy that ensures a degree of decentralization. In order to interact
with the smart contracts that have been deployed there is a `fabproxy` which
implements a limited set of APIs from the Ethereum JSON RPC API and therefore can be
used as a web3 provider.
