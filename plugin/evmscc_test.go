...
-----END CERTIFICATE-----`
		...
		Context("Instructor smart contract with event", func() {
			var (
...
-----END CERTIFICATE-----`
			...
			Context("when a contract has already been deployed", func() {
				...
				BeforeEach(func() {
					...
					mp = make([]evm_event.MessagePayload, 0)
					mp = append(mp, evm_event.MessagePayload{Message: msg})
				})
				...
				Context("if the method called emits event(s)", func() {
					It("sets the chaincode event", func() {
						...
						Expect(a[0].Message).To(Equal(msg))
					})
				})
